...
```{important}
Starting with the **26.04 LTS version** of the `landscape-server` charm, the deployment architecture changes to PostgreSQL 14+ over the `database` relation (backed by the `postgresql_client` charm interface), HAProxy 2.8 over `haproxy-route`, and TLS via `tls-certificates`.
...
For the 26.04 LTS architecture (recommended), the new deployment approach uses:
...
```{tip}
For the 26.04 LTS deployment, set `root_url` to a real hostname and point that hostname (via DNS or `curl --resolve`) at your HAProxy unit's IP address (or external load balancer) before testing.
```
